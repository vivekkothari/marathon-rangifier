...
public class ManagedServiceRegistry {
  ...
  private ConcurrentMap<ServiceInstance, ServiceProvider<ShardInfo>> instances =
      new ConcurrentHashMap<>();

  public void start() {
    Set<ServiceInstance> serviceInstances = marathonServiceClient.getInstances();
    Predicate<? super ServiceInstance> staleInstancePredicate =
        serviceInstance -> !serviceInstances.contains(serviceInstance);
    ...
    Map<ServiceInstance, ServiceProvider<ShardInfo>> serviceProviderMap =
        serviceInstances.stream()
            .collect(
                Collectors.toMap(
                    Function.identity(), serviceProviderBuilder::buildServiceProvider));
    serviceProviderMap
        .values()
        .forEach(
            provider -> {
